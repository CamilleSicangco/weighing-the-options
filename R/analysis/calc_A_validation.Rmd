...
First I define a version of the `calc_A` function that also outputs the value of Ci. This function is otherwise identical to `calc_A`.
...
Next we set up the environmental variables and physiological parameters.
...
Now we calculate the transpiration supply stream and the associated values of Tleaf, gs, A, and Ci.
``` {r calculations}
...
A_Ci_FvCB = calc_A_and_Ci(Tair, VPD, PPFD, E = E, net = TRUE, netOrig = TRUE, Ca = Ca, Wind = Wind, Wleaf = Wleaf)
```

To verify whether the system of equations is closed, we check if A and Ci from Fick's Law match A and Ci from FvCB (calculated above).
``` {r Ficks}
A_FvCB = unlist(A_Ci_FvCB[1])
Ci_FvCB = unlist(A_Ci_FvCB[2])

A_Ficks = g_w/1.57*(Ca - Ci_FvCB)
Ci_Ficks = Ca - A_FvCB * 1E-4 / (g_w/1.57) * 100^2
