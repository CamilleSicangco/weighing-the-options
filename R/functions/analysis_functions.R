  ...
  if(inputGS){
    ...
    if(!inputCi){
      ...
      getCI <- function(VJ,GSDIVA,PPFD,VPD,Ca,Tleaf,vpdmin,g0,Rd,
        ...
        # Modification assuming A = gs*(ca-ci) - Rd
        # Solution when Rubisco activity is limiting
        A <- g0 + GSDIVA * (Vcmax - Rd)
        B <- g0 * (Km - Ca) + GSDIVA * Rd * (Ca - Km) - 
          GSDIVA * Vcmax * (Ca + GammaStar) + Vcmax
        C <- Ca * Km * (GSDIVA * Rd - g0) + GammaStar * 
          (GSDIVA * Vcmax * Ca - 1.6)
        ...
        # Solution when electron transport rate is limiting
        A <- g0 + GSDIVA * (VJ - Rd)
        B <- g0 * (2.*GammaStar - Ca) + GSDIVA * Rd * (Ca - 2.*GammaStar) - 
          GSDIVA * VJ * (Ca + GammaStar) + VJ
        C <- Ca * 2.*GammaStar * (GSDIVA * Rd - g0) + GammaStar * 
          (GSDIVA * VJ * Ca - 1.6)
