...
composite_plot = function(df) {
  ...
  # Sperry et al. 2017
  p_A = df %>% 
    filter(ID %in% c("HC_constkmax", "CG_gross")) %>% 
    plot_costgain() + 
...
plot_costgain = function(df, separate = TRUE)
{
  palette = c(HC_constkmax = "#88CCEE", HC_varkmax = "#88CCEE", 
              TC = "#DDCC77", RC = "#117733",
              CG_net = "#CC6677", CG_net_newJT = "#CC6677", CG_gross = "#CC6677", CG_gross_varkmax = "#CC6677")
...
plot_profit = function(df) {
  ...
  df = df %>%  
    ...
    mutate(Sperry = CG_gross - HC_constkmax,
...
plot_physio_vs_Tleaf = function(df, all = TRUE, yvar = NULL, xvar = "Tleaf") {
  ...
  linetype = c(Sperry = "solid", "Sperry + varkmax" = "longdash", "Sperry + CGnet" = "dotted", "Sperry + CGnet + TC" = "dashed",
               Medlyn = "dotdash")
